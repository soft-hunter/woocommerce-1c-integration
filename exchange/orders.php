...
function wc1c_replace_document_products($order, $document_products) {
  ...
  foreach ($document_products as $document_product) {
    ...
    $item->set_subtotal($args['totals']['subtotal']);
    $item->set_total($args['totals']['total']);

    if (isset($args['variation'])) {
      $item->set_variation($args['variation']);
    }
...
function wc1c_replace_document($document) {
  ...
  if (!$order) {
    // Use new WooCommerce order creation method
    $order = new WC_Order();
    ...
    if (isset($user_id)) $order->set_customer_id($user_id);
    $order_id = $order->save();
    ...
    // Use CRUD API instead of wc_update_order
